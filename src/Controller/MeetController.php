...
class MeetController extends AbstractController
{
    ...
    public function index(MeetRepository $meetRepository, ParticipantRepository $participantRepository): Response
    {
    ...
    public function joinByCode(Request $request, MeetRepository $meetRepository): Response
    {
        ...
        if (!ctype_digit($code)) {
            $this->addFlash('error', 'Code de réunion invalide. Utilisez le numéro de la réunion.');
            return $this->redirectToRoute('app_meet_index');
        }

        $meet = $meetRepository->find((int) $code);
        if (!$meet) {
    ...
    public function room(Meet $meet): Response
    {
